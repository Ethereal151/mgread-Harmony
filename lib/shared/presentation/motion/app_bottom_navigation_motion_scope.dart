...
///
/// 职责：
/// - 持有跨路由选中胶囊的唯一 AnimationController。
...
abstract interface class AppBottomNavigationMotion {
  ...
  AppNavigationDestination visualSelectionFor(AppNavigationDestination fallback);
}
...
class AppBottomNavigationMotionScope extends StatefulWidget {
  ...
  const AppBottomNavigationMotionScope({required this.child, this.initialDestination, super.key});

  final Widget child;
  final AppNavigationDestination? initialDestination;

  static AppBottomNavigationMotion? maybeOf(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<_AppBottomNavigationMotionInherited>()?.motion;
  }

  @override
  State<AppBottomNavigationMotionScope> createState() => _AppBottomNavigationMotionScopeState();
}

class _AppBottomNavigationMotionScopeState extends State<AppBottomNavigationMotionScope>
  ...
  void initState() {
    super.initState();
    _controller = AnimationController(duration: AppMotion.bottomNavigationPillTravel, vsync: this);
  ...
  AppNavigationDestination visualSelectionFor(AppNavigationDestination fallback) {
    ...
    if (_controller.isAnimating && _controller.value < AppMotion.bottomNavigationSelectionHandoff) {
  ...
  void animateTo(AppNavigationDestination destination) {
    ...
    final AppNavigationDestination currentVisual = visualSelectionFor(_target ?? destination);
    ...
    final double overshoot = target + direction * AppMotion.bottomNavigationPillOvershoot;
    _position = TweenSequence<double>(<TweenSequenceItem<double>>[
      TweenSequenceItem<double>(tween: ConstantTween<double>(current), weight: 14),
      TweenSequenceItem<double>(
        tween: Tween<double>(begin: current, end: overshoot).chain(CurveTween(curve: AppMotion.navigationCurve)),
        weight: 70,
      ),
      TweenSequenceItem<double>(
        tween: Tween<double>(begin: overshoot, end: target).chain(CurveTween(curve: AppMotion.navigationCurve)),
        weight: 16,
      ),
    ]).animate(_controller);
  ...
  TweenSequence<double> _pillScaleSequence({required double begin, required double travelling, required double arrival}) {
    return TweenSequence<double>(<TweenSequenceItem<double>>[
      TweenSequenceItem<double>(
        tween: Tween<double>(begin: begin, end: travelling).chain(CurveTween(curve: AppMotion.standardCurve)),
        weight: 18,
      ),
      TweenSequenceItem<double>(tween: ConstantTween<double>(travelling), weight: 45),
      TweenSequenceItem<double>(
        tween: Tween<double>(begin: travelling, end: arrival).chain(CurveTween(curve: AppMotion.navigationCurve)),
        weight: 25,
      ),
      TweenSequenceItem<double>(
        tween: Tween<double>(begin: arrival, end: 1).chain(CurveTween(curve: AppMotion.navigationCurve)),
        weight: 12,
      ),
    ]);
  }
  ...
  Widget build(BuildContext context) {
    return _AppBottomNavigationMotionInherited(motion: this, child: widget.child);
  }
}

class _AppBottomNavigationMotionInherited extends InheritedWidget {
  const _AppBottomNavigationMotionInherited({required this.motion, required super.child});
