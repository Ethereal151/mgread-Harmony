...
- 2026-09-22/23 AudioSession 增量适配：`MgReadOhosMediaPlugin.ets` 已按 API 26 官方 AudioSession 路径设置媒体场景、激活 `CONCURRENCY_DEFAULT`、监听 PAUSE/STOP/TIME_OUT_STOP/RESUME，并将所有 AVPlayer（音频和视频）设为 `SHARE_MODE`；视频使用公开的 `AVSessionType='video'`，音频使用 `audio`；`openAudio/openVideo(loop: true)` 仅用于稳定的长时验收探针，默认播放行为不变。arm64 HAP 构建、慢速安装和局域网端点 `192.168.1.8:45975`（`PLA-AL10`/`aarch64`）启动均通过；系统 QQ 音乐抢占音频时捕获 `OHOS_AUDIO_INTERRUPT_EVENT=interrupted:true`，并同步捕获 `DeactivateAudioSessionInFakeFocusMode` 向 MgRead PID 发送中断回调。该 API 26 设备在第三方播放器暂停/强停后不发送自动 `RESUME`；用户主动播放兜底已在 `01:15:00` 捕获音频 `interrupted:false`、`state:playing`、`playing:true`，并在 `01:27:13`/`01:27:17` 的视频专项中捕获视频 `interrupted:true`→`interrupted:false`、`state:playing`、`playing:true`，因此音视频均具备可验证的稳定恢复路径。
- 2026-09-22 系统代理专项诊断：在同一 `PLA-AL10` arm64/API 26 设备上分别尝试回环代理和局域网主机代理，并在每次测试后清除设备代理、恢复 HDC；当时应用通过生产路径读取 `connection.getDefaultHttpProxy()` 均得到空值，因此未将空值误报为通过。该中间结论已由 2026-09-23 的真实 WLAN 代理、HTTPS CONNECT 与非 loopback `NO_PROXY` 验收补充覆盖。
- 2026-09-23 系统代理与非 loopback `NO_PROXY` 真机验收补齐：在 `PLA-AL10`/`aarch64`/API 26、`192.168.1.8:45975` 上将 WLAN 手动代理设置为 `192.168.1.3:35555`，排除项设置为 `example.com`；更新后的 `ohos_runtime_https_proxy_smoke_test.dart` arm64 HAP 输出 `All tests passed!`，外部 CONNECT 代理收到未排除流量但没有 `example.com:443`，验证系统代理读取、HTTPS CONNECT 与非 loopback 排除项直连。测试结束后设置页确认代理恢复为“无”，HDC 重连成功。
...
| 来源网络/代理 | `pass` | OHOS HTTP、系统/自定义来源代理单测；固定 Node `--jitless` 四项回归通过，包含原生直连、gzip、显式 HTTP/SOCKS5、环境 HTTP 代理、NO_PROXY 和 Runtime 配置切换；无 WebAssembly fallback 已补原生直连、HTTP 代理和 SOCKS5 代理通道并支持 gzip 解压；`ohos_real_source_smoke_test.dart` 在 `PLA-AL10` arm64 真机通过 35ge、德奇、番茄、米读、书库 365 五个真实来源的插件导入、发现、搜索、详情、目录和正文链路；`ohos_browser_proxy_smoke_test.dart` 在同一 arm64 真机通过本地 LAN HTTP 代理命中验证；`ohos_runtime_https_proxy_smoke_test.dart` 在同一真机无 WASM Runtime 的 HTTPS CONNECT 实际代理链路通过，并以系统 WLAN 代理 + `example.com` 排除项验证非 loopback `NO_PROXY` | 播放器代理继续按 OHOS SDK 不支持处理 |
...
| 视频 | `pass` | arm64 真机 Texture、首帧、进度、`CACHED_DURATION -> bufferedPosition`、OHOS 窗口全屏/方向切换/恢复、Unified Streaming H.264 HLS 播放和外部 QQ 音乐抢占/用户主动恢复 `interrupted:true`→`interrupted:false`→`playing:true` 均通过；x64 复验也通过；Apple BipBop HLS 返回 `open_failed / avplayer_state_error`，Mux x36xhzz HLS 可打开但 `play` 超时，Mux tears-of-steel HLS 返回 `open_failed / avplayer_state_error`，均保留为兼容性边界 | OHOS 播放器代理无公开 SDK 契约，按 `unsupported` 稳定降级；全局系统音量已按官方 SDK 边界稳定 `unsupported`，不要求伪造系统音量写入证据；HLS 已有 arm64 成功样本但外部流兼容性不统一 |
...
因此，“计划内可实现项”的代码实现已完成；上面列出的 OHOS SDK 能力边界属于有证据的 `unsupported`，不是遗留的静默空操作。当前总验收状态为 `pass`：播放器代理仍是官方 SDK 边界并已稳定降级，系统代理实际切换、HTTPS CONNECT 与非 loopback `NO_PROXY` 均有 arm64 真机证据；音频和视频外部抢占及用户主动恢复均已通过。真实来源→书架→Reader 数据迁移、文件导入/导出选择器、文件分享分发、HTTPS 外链桥接、反馈页目标仓库页面、有效二维码业务路由、来源代理、HLS、真实跨设备同步和 HAP 传输已补齐对应证据，OHOS↔OHOS 与不上架的 HAP 市场跳转均已按用户要求明确跳过，不计入本轮阻塞项。
