...
- 2026-09-22/23 AudioSession 增量适配：`MgReadOhosMediaPlugin.ets` 已按 API 26 官方 AudioSession 路径设置媒体场景、激活 `CONCURRENCY_DEFAULT`、监听 PAUSE/STOP/TIME_OUT_STOP/RESUME，并将 AVPlayer 设为 `SHARE_MODE`；`openAudio(loop: true)` 仅用于稳定的长时验收探针，默认播放行为不变。arm64 HAP 构建、慢速安装和局域网端点 `192.168.1.8:45975`（`PLA-AL10`/`aarch64`）启动均通过；系统音乐抢占时捕获 `OHOS_AUDIO_INTERRUPT_EVENT=interrupted:true`，并同步捕获 `DeactivateAudioSessionInFakeFocusMode` 向 MgRead PID 发送中断回调。音乐应用释放焦点后的 `interrupted:false` 尚未捕获，因此本项只把外部抢占标记为已验证，恢复仍保留为未完成证据。
