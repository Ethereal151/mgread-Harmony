...
| 表面 | 状态 | 当前证据 | 尚缺证据或边界 |
| --- | --- | --- | --- |
| Runtime | `pass` | `ohos_runtime_smoke_test.dart` 在 arm64 真机通过 Node host、Runtime ping、Network Kit 地址和能力降级；x64 模拟器仍稳定返回 `runtime_architecture_unavailable` | 取消/重启/插件卸载等完整生命周期仍需独立真机用例 |
