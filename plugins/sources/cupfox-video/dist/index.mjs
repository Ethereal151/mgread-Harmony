...
export async function discover(request) { if (request.target === null) {
    ...
    throw new Error('Discovery target is invalid.'); if (request.cursor !== null || request.collectionId !== null)
    throw new Error('Discovery continuation is not supported.'); const limit = clamp(request.pageSize), url = `${base}/type/${category[0]}/`, values = parseList(await fetchText(url)).slice(0, limit), collectionId = `video:${category[0]}`, items = values.map(content => frozen({ content, rank: null, metric: null, recommendation: null })), continuation = null; return frozen({ kind: 'document', document: { components: [{ type: 'section', id: `${collectionId}:section`, title: category[1], subtitle: null, icon: 'video', children: [{ type: 'contentCollection', id: collectionId, layout: 'coverGrid', items, continuation }] }] } }); }
