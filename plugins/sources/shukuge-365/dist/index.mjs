...
var categories = Object.freeze([
  ...
  ["transmigration", "穿越", "/i-chuanyue/"],
  ["rebirth", "重生", "/i-chongsheng/"],
  ["alternate-history", "架空", "/i-jiakong/"],
  ["ceo", "总裁", "/i-zongcai/"],
  ["wuxia", "武侠", "/i-wuxia/"],
  ["xianxia", "仙侠", "/i-xianxia/"],
  ["danmei", "耽美", "/i-danmei/"],
  ["urban", "都市", "/i-dushi/"],
  ["military", "军事", "/i-junshi/"],
  ["game", "网游", "/i-wangyou/"],
  ["mystery", "悬疑", "/i-xuanyi/"],
  ["literature", "文学", "/i-wenxue/"],
  ["science-fiction", "科幻", "/i-kehuan/"],
  ["cultivation", "修真", "/i-xiuzhen/"],
  ["history", "历史", "/i-lishi/"],
  ["other", "其他", "/i-qita/"],
  ["new", "最新小说", "/new/"],
  ["ranking", "排行榜", "/top/"]
...
var ShukugeSource = class {
  ...
  parseList(html3, pageUrl) {
    ...
    $(".listitem").each((_, element) => {
      ...
      const latestLink = root2.find(".bookdesc p.desc").first().find("a[href]").first();
      const latestTitle = clean(latestLink.text());
      const latestHref = latestLink.attr("href");
      const latestUrl = latestHref === void 0 ? null : new URL(latestHref, pageUrl);
      const image = root2.find("a.cover img").first().attr("src");
      const description = cleanDescription(root2.find(".bookdesc p.desc").eq(1).text());
      items.push(summary({
        url,
        title,
        author,
        coverUrl: image === void 0 ? null : this.#proxyImage(new URL(image, pageUrl), pageUrl),
        description,
  ...
  async getDetail(id) {
    ...
    return Object.freeze({
      ...summary({
        ...
        title,
        author,
        coverUrl: coverRaw === void 0 ? null : this.#proxyImage(new URL(coverRaw, url), url),
        description: cleanDescription($(".bookdtext > p").first().text()),
        status: parseStatus(statusText),
        updatedAt,
        latestTitle: clean(latest.text()),
        latestUrl: latest.attr("href") === void 0 ? null : new URL(latest.attr("href"), url),
        categories: category === null ? [] : [category]
      }),
  ...
  async getChapters(id) {
    ...
    const seen = /* @__PURE__ */ new Set();
    const chapters = [];
    for (const html3 of pages) {
      const page = load(html3);
...
function encodeBookId(url) {
  return `book:${token(normalizeBookUrl(url).pathname)}`;
}
function encodeChapterId(url) {
  return `chapter:${token(url.pathname)}`;
}
...
function normalizeBookUrl(url) {
  return new URL(url.pathname.replace(/\/?$/u, "/"), origin);
}
function isBookUrl(url) {
  return url.origin === origin && /^\/book\/\d+\/?$/u.test(url.pathname);
}
function isCatalogUrl(url, bookUrl) {
  return url.origin === origin && url.pathname.startsWith(normalizeBookUrl(bookUrl).pathname) && /(?:index|index_\d+)\.html$/u.test(url.pathname);
}
function isChapterUrl(url, bookUrl) {
...
function stripLabel(value, label) {
  return value === null ? null : clean(value.replace(new RegExp(`^${label}[：:]?\\s*`, "u"), ""));
}
