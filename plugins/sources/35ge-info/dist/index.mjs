...
var ThirtyFiveSource = class {
  ...
  #parseRows($, selector, pageUrl, fallbackCategory) {
    ...
    for (const element of $(selector).toArray()) {
      ...
      items.push(summary({
        url,
        title,
        author: clean(root2.find(".s4, .s5").last().text()),
        coverUrl: coverUrl === null ? null : this.#proxyImage(coverUrl, pageUrl),
        description: null,
        status: parseStatus(clean(root2.find(".s7").first().text())),
        updatedAt: null,
        latestTitle: clean(latestLink.text()),
        latestUrl: latestHref === void 0 ? null : new URL(latestHref, pageUrl),
  ...
  #proxyImage(url, referer) {
    return url.origin === origin && referer.origin === origin ? this.context.resource.proxy({ kind: "image", url: url.toString(), headers: { Accept: "image/*", Referer: referer.toString() } }) : null;
  }
};
...
function encodeBookId(url) {
  return `book:${token(bookIdentity(url))}`;
}
...
function isBookUrl(url) {
  return url.origin === origin && /^\/(?:xs\/)?\d+\/\d+\/?$/u.test(url.pathname);
}
function bookIdentity(url) {
  const match = /^\/(?:xs\/)?(\d+\/\d+)\/?$/u.exec(url.pathname);
  return match?.[1] === void 0 ? "" : `/${match[1]}/`;
}
function inferredCoverUrl(url) {
  const match = /^\/(?:xs\/)?(\d+)\/(\d+)\/?$/u.exec(url.pathname);
  return match?.[1] === void 0 || match[2] === void 0 ? null : new URL(`/files/article/image/${match[1]}/${match[2]}/${match[2]}s.jpg`, origin);
}
function sameBookChapter(chapter, book) {
  const key = bookIdentity(book);
  return key !== "" && chapter.origin === origin && new RegExp(`^/xs?${key.replaceAll("/", "\\/")}\\d+\\.html$`, "u").test(chapter.pathname);
}
