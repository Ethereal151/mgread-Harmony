...
async function search(request) {
  ...
  const values = await withPage(async (browser) => {
    await browser.navigate(base, { timeoutMs: 3e4 });
    const token = await browser.executeJavaScript(`document.querySelector('input[name="t"]')?.value||''`, { timeoutMs: 1e4 });
    const searchToken = typeof token === "string" ? token : "";
    await browser.navigate(`${base}/search?t=${encodeURIComponent(searchToken)}&k=${encodeURIComponent(query)}&page=${page}`, { timeoutMs: 3e4 });
...
async function discover(request) {
  ...
  const limit = clamp(request.pageSize);
  const values = await withPage(async (browser) => {
    await browser.navigate(`${base}/show/${category[2]}------${page}.html`, { timeoutMs: 35e3 });
...
async function getChapters(request) {
  ...
  const groups = [...grouped.entries()].map(([line, rows], groupOrder) => {
    ...
    const projected = rows.map((episode, order) => frozen({
      id: `ncat:${id}:${line}:${episode.episode}`,
      title: episode.title,
      order,
      url: playUrl(id, line, episode.episode),
      volumeTitle: title,
      wordCount: null,
      updatedAt: null,
      isLocked: null,
...
async function getContent(request) {
  ...
  for (let captureAttempt = 0; captureAttempt < 2 && upstream === ""; captureAttempt += 1) {
    const mediaUrl = await withPage(async (browser) => {
      await browser.navigate(pageUrl, { timeoutMs: 35e3 });
...
async function readDetail(browser) {
  ...
  const value = {
    ...
    author: clean(text(raw.author)),
    latest: clean(text(raw.latest)),
