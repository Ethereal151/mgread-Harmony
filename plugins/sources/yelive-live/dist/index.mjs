...
// dist/index.mjs
var root = "https://zh.yelive.tv/";
var agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/136.0.0.0 Safari/537.36";
var headers = { "User-Agent": agent, Accept: "application/json,text/plain,*/*", "Accept-Language": "zh-CN,zh;q=0.9,en;q=0.8", "Front-Version": "11.7.45", Referer: root };
...
async function search(request) {
  ...
  for (const primary of ["girls", "couples", "men", "trans"]) {
  ...
  const items = [...result.values()].slice(0, clamp(request.pageSize));
...
async function discover(request) {
  if (request.target === null)
    return frozen({ kind: "document", document: { components: [{ type: "section", id: "yelive-channels", title: "全网直播", subtitle: "Yelive 公开房间", icon: "live", children: [{ type: "categoryCollection", id: "yelive-channel-list", layout: "chips", categories: channels.map((channel2) => ({ id: channel2.id, title: channel2.title, target: `channel:${channel2.id}`, count: null, url: null, icon: "live" })) }] }] } });
...
async function getDetail(request) {
  ...
  return frozen({ ...item, aliases: [], catalogUrl: `${root}${encodeURIComponent(model.username)}` });
}
async function getChapters(request) {
  const id = contentId(request.id), model = await locate(id), qualities = qualityList(model), items = qualities.map((quality, index) => frozen({ id: `live:${id}:${quality}`, title: quality === "auto" ? "自动清晰度" : quality.toUpperCase(), order: index, url: null, volumeTitle: "低延迟直播", wordCount: null, updatedAt: null, isLocked: false, attributes: [] }));
  return frozen({ items, groups: [frozen({ id: `group:${id}:live`, title: "低延迟直播", order: 0, episodes: items })] });
}
async function getContent(request) {
  const id = contentId(request.id), quality = chapterQuality(request.chapterId, id), model = await locate(id), upstream = hlsUrl(model, quality);
  if (!safeUrl(upstream))
    throw new Error("Live stream is unavailable.");
  const mediaHeaders = { "User-Agent": agent, Accept: "*/*", Origin: root.slice(0, -1), Referer: `${root}${encodeURIComponent(model.username)}` };
...
function models(value) {
  const rows = findRows(value, 0), result = [];
...
function findRows(value, depth) {
  ...
  if (Array.isArray(value)) {
    ...
    if (objects.some((row) => row.id !== void 0 || row.username !== void 0))
      return objects;
  ...
  if (!isObject(value))
    return [];
  for (const key of ["models", "items", "data", "result", "list", "group", "payload", "blocks"]) {
...
function summary(model) {
  return frozen({ id: `live:${model.id}`, title: model.username, contentKind: "video", coverOrientation: "portrait", author: model.viewers ? `${model.viewers} 人观看` : "Yelive", url: `${root}${encodeURIComponent(model.username)}`, coverUrl: proxyImage(model.cover), description: `状态：${model.status}`, language: null, status: "ongoing", access: "free", wordCount: null, chapterCount: qualityList(model).length, publishedAt: null, updatedAt: null, latestChapter: { id: `live:${model.id}:auto`, title: "正在直播", url: null, updatedAt: null }, categories: [], tags: [], attributes: [] });
}
function qualityList(model) {
  const known = model.presets.filter((value) => /^(?:1080p|720p|480p|240p)$/u.test(value));
  return ["auto", ...new Set(known)].slice(0, 5);
}
function hlsUrl(model, quality) {
  const candidate = model.hls || `https://edge-hls.doppiocdn.com/hls/${model.id}/master/${model.id}_240p.m3u8`, url = new URL(candidate), host = url.host || "edge-hls.doppiocdn.com", suffix = quality === "auto" ? "auto" : quality;
  return `https://${host}/hls/${model.id}/master/${model.id}_${suffix}.m3u8?playlistType=lowLatency`;
}
function absolute(value) {
  if (value.startsWith("//"))
    return `https:${value}`;
  if (value.startsWith("/"))
    return `https://static-proxy.strpst.com${value}`;
  return value;
}
function contentId(id) {
  const value = /^live:(\d+)$/u.exec(id)?.[1];
  if (!value)
    throw new Error("Content ID is invalid.");
  return value;
}
function chapterQuality(id, content) {
  const value = new RegExp(`^live:${content}:(auto|1080p|720p|480p|240p)$`, "u").exec(id)?.[1];
  if (!value)
    throw new Error("Chapter ID is invalid.");
  return value;
}
function proxyImage(url) {
  return safeUrl(url) ? requireContext().resource.proxy({ kind: "image", url, headers: { Referer: root } }) : null;
}
