...
var DiyibanzhuSource = class {
  ...
  async search(query) {
    ...
    return this.parseList(html3, url);
  }
  async discover(id, page) {
    ...
    const url = page === 1 ? firstUrl : paginationUrl(firstHtml, firstUrl, page);
    const html3 = page === 1 ? firstHtml : await this.#cache.getOrFetchText(url, listingPolicy, () => this.#browser(url, "GET", null));
    this.context.log.info("source_discover_listing_received");
    const items = this.parseList(html3, url);
    this.context.log.info("source_discover_listing_parsed");
    return items;
  }
  parseList(html3, base) {
    ...
    $("li.column-2").each((_, element) => {
      ...
      const latest = clean(root2.find(".update a").text());
      const words = parseCount(root2.find(".words").text()) ?? parseCount(info);
      items.push(summary(url, title, author, null, null, words, latest, []));
    });
  ...
  async detail(id) {
    ...
    const statusText = clean($(".mod.detail .status").text());
    const status = statusText?.includes("完结") === true ? "completed" : statusText?.includes("连载") === true ? "ongoing" : "unknown";
    const intro = clean($(".mod.book-intro .bd").text());
    const latest = clean($(".mod.block.update.chapter-list .bd li a").first().text());
    const coverRaw = $(".mod.detail img,.detail img").first().attr("src");
    const cover = coverRaw === void 0 ? null : this.#proxy(new URL(coverRaw, url), url);
    return Object.freeze({ ...summary(url, title, author, cover, intro, null, latest, kind === null ? [] : [kind]), aliases: Object.freeze([]), catalogUrl: url.toString(), status });
  }
  async chapters(id) {
    ...
    for (let page = 0; page < 100; page += 1) {
      const html3 = await this.#cache.getOrFetchText(url, catalogPolicy, () => this.#browser(url, "GET", null));
      const $ = load(html3);
...
function summary(url, title, author, coverUrl, description, wordCount, latest, categories2) {
...
function isBook(url) {
  return url.origin === origin && (/\/list\/\d+(?:_\d+)*\.html$/u.test(url.pathname) || url.pathname === "/wap.php" && url.searchParams.get("action") === "list" && /^\d+$/u.test(url.searchParams.get("id") ?? ""));
}
function isChapter(url) {
  return url.origin === origin && (/\/view\/\d+(?:_\d+)?\.html$/u.test(url.pathname) || url.pathname === "/wap.php" && url.searchParams.get("action") === "article" && /^\d+$/u.test(url.searchParams.get("id") ?? ""));
}
function paginationUrl(html3, base, page) {
  const $ = load(html3);
  let template;
...
function isCf(body) {
  return /(?:cf-challenge|cf-turnstile|Just a moment|Checking your browser)/iu.test(body);
}
...
async function getDetail(request) {
  return invoke("detail", () => requireSource().detail(request.id));
}
async function getChapters(request) {
  return invoke("chapters", () => requireSource().chapters(request.id));
}
