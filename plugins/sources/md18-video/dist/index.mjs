...
var categories = Object.freeze([["1", "麻豆视频"], ["2", "日本视频"], ["3", "欧美视频"], ["4", "动漫视频"], ["5", "国产视频"]]);
var context;
...
async function getDetail(request) {
  ...
  const html = await fetchText(detailUrl(id));
  return detail(parseDetail(html, id));
}
...
function parseDetail(html, id) {
  const title = firstText(html, /<h1[^>]*>([\s\S]*?)<\/h1>/iu) || firstText(html, /<title[^>]*>([\s\S]*?)<\/title>/iu);
  const cover = firstAttribute(html, /<meta[^>]+property=["']og:image["'][^>]*>/iu, "content") || firstAttribute(html, /<img\b[^>]*>/iu, "data-original") || firstAttribute(html, /<img\b[^>]*>/iu, "src");
...
function parseGroups(html, id) {
  ...
  if (fallback.episodes.length === 0) throw new Error("No playable episodes found.");
  return [fallback];
}
...
function groupFromLinks(html, id, title, order) {
  const episodes = links(html).filter((entry) => entry.id === id).map((entry, index) => frozen({ id: `video:${id}:${entry.sid}:${entry.nid}`, title: entry.title || `第${index + 1}集`, order: index, url: playUrl(id, entry.sid, entry.nid), volumeTitle: decode(title), wordCount: null, updatedAt: null, isLocked: null, attributes: [] }));
  const sid = episodes[0]?.id.split(":")[2] ?? String(order + 1);
...
function playerUrl(data) {
  const raw = text(data.url);
  const encrypt = Number(data.encrypt ?? 0);
  const decoded = encrypt === 2 ? Buffer.from(raw, "base64").toString("utf8") : raw;
  return legacyDecode(decoded);
}
...
function categoryUrl(id, page) {
  return page <= 1 ? `${base}/index.php/vod/type/id/${id}.html` : `${base}/index.php/vod/show/id/${id}/page/${page}.html`;
}
function searchUrl(query, page) {
  const suffix = page <= 1 ? "" : `&page=${page}`;
  return `${base}/index.php/vod/search.html?wd=${encodeURIComponent(query)}${suffix}`;
}
function detailUrl(id) {
  return `${base}/index.php/vod/detail/id/${id}.html`;
}
function playUrl(id, sid, nid) {
  return `${base}/index.php/vod/play/id/${id}/sid/${sid}/nid/${nid}.html`;
}
...
function proxyImage(value) {
  ...
  return url === null ? null : requireContext().resource.proxy({ kind: "image", url, resourceTransform: "sniff-image-content-type-v1", headers: { Referer: `${base}/`, "User-Agent": headers["User-Agent"] } });
}
function attribute(text2, name) {
  return new RegExp(`${escape(name)}=["']([^"']+)["']`, "iu").exec(text2)?.[1] ?? "";
}
