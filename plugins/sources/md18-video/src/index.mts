...
function parseGroups(html: string, id: string) {
  const blocks = playlistBlocks(html).filter((block) => links(block.html).some((entry) => entry.id === id)); const groups = blocks.map((block, index) => groupFromLinks(block.html, id, block.title || `分组 ${index + 1}`, index));
