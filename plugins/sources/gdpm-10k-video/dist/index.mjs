...
async function getDetail(request) {
  const id = contentId(request.id), value = object(await videoInfo(id)), item = summary(value);
  return frozen({ ...item, description: text(value.vod_name) || item.description, aliases: [], catalogUrl: item.url });
}
async function getChapters(request) {
  const id = contentId(request.id), value = object(await videoInfo(id)), title = text(value.vod_duration) || "正片", chapter = frozen({ id: `video:${encodeKey(id)}:main`, title, order: 0, url: null, volumeTitle: "默认线路", wordCount: null, updatedAt: null, isLocked: null, attributes: [] });
  return frozen({ items: [chapter], groups: [frozen({ id: `group:${encodeKey(id)}:default`, title: "默认线路", order: 0, episodes: [chapter] })] });
}
...
async function categoryList() {
  const data = await api("/data/navigation"), values = records(data), result = [], seen = /* @__PURE__ */ new Set();
