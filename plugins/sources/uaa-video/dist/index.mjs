...
// src/index.mts
var mainOrigin = "https://www.uaa.com";
var apiOrigin = "https://www.uaa001.com";
var apiRoot = apiOrigin + "/api/video/app/video/";
var minimumDelayMs = 300;
...
var coverHeaders = Object.freeze({
...
async function discover(request) {
  ...
  const continuation = hasNextPage(result, page, values.length, size) ? frozen({ target: request.target, cursor: "channel:" + channel.id + ":" + (page + 1) }) : null;
...
async function rootDocument(pageSize) {
  ...
  if (latest === void 0) throw new Error("Source channels are unavailable.");
...
function cursorPage(cursor, prefix) {
  ...
  if (!Number.isSafeInteger(value) || value < 2 || value > 1e4) throw new Error("Source cursor is invalid.");
  return value;
}
function hasNextPage(result, requestedPage, itemCount, pageSize) {
  ...
  return itemCount >= pageSize && requestedPage < 1e4;
}
...
function createViewId() {
  return String(Date.now()) + String(Math.floor(Math.random() * 9e3 + 1e3));
}
