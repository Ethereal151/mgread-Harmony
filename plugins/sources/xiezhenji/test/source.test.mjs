...
test('fixture chain uses one browser session for discovery, detail and gallery HTML', async (t) => {
  ...
  assert.equal(info.chapterCount, 16);
  const chapters = await plugin.getChapters({ id: info.id });
  assert.equal(chapters.items.length, 16);
  assert.equal(chapters.items[0].title, '第1页');
  assert.equal(chapters.items.at(-1).title, '第16页');
  const firstContent = await plugin.getContent({ id: info.id, chapterId: chapters.items[0].id });
  assert.equal(firstContent.pages.length, 1);
  const secondContent = await plugin.getContent({ id: info.id, chapterId: chapters.items[1].id });
  assert.equal(secondContent.pages.length, 1);
  assert.equal(browserRequests.filter((request) => /\/article\/42\/page\/\d+\/$/u.test(new URL(request.url).pathname)).length, 1);
