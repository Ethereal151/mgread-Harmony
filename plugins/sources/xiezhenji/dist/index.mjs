...
var XiezhenjiSource = class {
  ...
  parseList(html3, base) {
    ...
    const values = [];
    $("article.excerpt,article.homepage-content-card").each((_, element) => {
      const root2 = $(element);
      ...
      values.push(makeSummary(url, title, raw === void 0 ? null : this.#proxy(new URL(raw, base), base), clean(root2.find(".note, .excerpt-note, .post-excerpt").text()), unique([root2.find("a.imgbox-a,.homepage-content-card__type").text()])));
    });
  ...
  async detail(id) {
    ...
    const count = parseImageCount(html3, description);
    const total = parseTotalPages(html3);
    return Object.freeze({ ...makeSummary(url, title.replace(/\s*-\s*爱妹子\s*$/u, ""), rawCover === null ? null : this.#proxy(new URL(rawCover, url), url), description, tags, total), aliases: Object.freeze([]), catalogUrl: url.toString(), author: tags[0] ?? null, attributes: count === null ? Object.freeze([]) : Object.freeze([{ key: "images", label: "图片", value: String(count) }]) });
  }
  async chapters(id) {
    const url = decodeId(id);
    const total = parseTotalPages(await this.#cached(url, detailPolicy));
    return Object.freeze({ items: chapterItems(url, total) });
  }
  async content(id, chapterId2) {
    const base = decodeId(id);
    const page = parseChapterPage(base, chapterId2);
  ...
  async #cached(url, policy) {
    return this.#cache.getOrFetchText(url, policy, () => this.#html(url));
  }
  #html(url) {
    const operation = this.#browserTail.then(() => this.#browserHtml(url));
    this.#browserTail = operation.then(() => void 0, () => void 0);
    return operation;
  }
  async #browserHtml(url) {
    const page = await this.#page();
    await this.#ready(page);
    let response = await page.fetch({ url: url.toString(), method: "GET", headers: { accept: "text/html,application/xhtml+xml" }, body: null, responseType: "text", timeoutMs: browserTimeoutMs });
...
function makeSummary(url, title, coverUrl, description, tags, chapterCount = 1) {
  const latestPage = Math.max(1, chapterCount);
...
function chapterId(url, page) {
  return `gallery:${token(url)}:${page}`;
}
function galleryPageUrl(base, page) {
  return page === 1 ? base : new URL(`${base.pathname.replace(/\/?$/u, "/")}page/${page}/`, origin);
}
function chapterItems(url, total) {
  return Object.freeze(Array.from({ length: total }, (_, index2) => {
    const page = index2 + 1;
    return Object.freeze({ id: chapterId(url, page), title: `第${page}页`, order: index2, url: galleryPageUrl(url, page).toString(), volumeTitle: null, wordCount: null, updatedAt: null, isLocked: false, attributes: Object.freeze([]) });
  }));
}
...
function parseTotalPages(html3) {
  const declared = Number(/(?:["']?)(?:total_pages|totalPages)(?:["']?)\s*[:=]\s*["']?(\d+)/u.exec(html3)?.[1]);
  const $ = load(html3);
  const linked = [...$("a").toArray().map((element) => clean($(element).text())), ...$("[data-page]").toArray().map((element) => $(element).attr("data-page"))].map((value) => Number(value)).filter((value) => Number.isSafeInteger(value) && value > 0 && value <= 80);
  const count = Math.max(1, Number.isSafeInteger(declared) ? declared : 1, ...linked);
  return Math.min(80, count);
}
function parseImageCount(html3, intro) {
  const count = Number(/"numberOfItems"\s*:\s*(\d+)/u.exec(html3)?.[1] ?? /收录\s*(\d+)\s*P/iu.exec(intro ?? "")?.[1]);
  return Number.isSafeInteger(count) && count >= 0 ? count : null;
}
function parseImages(html3, base) {
  const $ = load(html3);
...
function mime(url) {
  const ext = /\.([^.]+)$/u.exec(url.pathname)?.[1]?.toLowerCase();
...
async function search(request) {
  const page = cursor(request.cursor, "search");
  const items = (await requireSource().search(request.query, page)).slice(0, request.pageSize);
  return Object.freeze({ items, nextCursor: items.length === request.pageSize ? `search:${page + 1}` : null, totalCount: null });
}
