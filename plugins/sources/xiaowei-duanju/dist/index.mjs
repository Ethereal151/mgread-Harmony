...
async function getDetail(request) {
  ...
  const first2 = episodes[0] ?? {};
  const item = summaryFrom({ ...first2, ...json, oneId: id });
  return frozen({ ...item, description: text(json.description) || text(first2.description) || null, aliases: [], catalogUrl: detailUrl(id) });
}
...
async function detailJson(id) {
  return fetchJson(`/shortVideoDetail?oneId=${encodeURIComponent(id)}&page=1&pageSize=500&userId=0&queryAll=true`);
}
...
function commonQuery() {
  return `version_code=1061&version_name=1.0.61&device_name=${encodeURIComponent("Pixel 8 Pro")}&device_type=phone&is_first_day=true&is_first_24h=true&app_launch_way=icon&default_homepage=homepage_interaction&device_owning_firm=Google&font_scale=default&os_type=1&clientInfo=${clientInfo}`;
}
function detailUrl(id) {
  return `${base}/shortVideoDetail?oneId=${encodeURIComponent(id)}`;
}
function detailEpisodes(json) {
  const direct = records(json.data);
  return direct.length > 0 ? direct : records(object(json.data).list);
}
...
function proxyImage(value) {
  ...
  return safeUrl(url) ? requireContext().resource.proxy({ kind: "image", url, headers: { "User-Agent": requestHeaders["User-Agent"] } }) : null;
}
...
function unique(value) {
  return [...new Set(strings(value).map((item) => item.trim()).filter(Boolean))];
}
