...
${url.toString()}`, "utf8").digest("hex");
  ...
  async #read(key) {
    try {
      ...
      if (!metadata2.isFile() || metadata2.size > maximumEntryBytes) {
        return void 0;
      }
...
var catalogPolicy = Object.freeze({
  namespace: "catalog",
  staleAfterMs: 60 * 60 * 1e3,
  allowStaleOnError: false
});
...
var DouyinXiaoshuoSource = class {
  ...
  async search(query) {
    ...
    return this.#parseListing(html3, url, null).items;
  }
  ...
  async getDetail(id) {
    ...
    const coverRaw = metadata($, "og:image") ?? clean($(".synopsisArea_detail img").first().attr("src"));
    const updateTime = metadata($, "og:novel:update_time");
    const coverUrl = coverRaw === null ? null : this.#proxyImage(new URL(coverRaw, url), url);
    const attributes2 = updateTime === null ? Object.freeze([]) : Object.freeze([
      Object.freeze({
  ...
  #parseListing(html3, baseUrl, fallbackCategory) {
    const $ = load(html3);
    const seen = /* @__PURE__ */ new Set();
    const items = [];
    $(".bookbox,.recommend .hot_sale").each((_, element) => {
      ...
      if (link.length === 0) {
        link = root2.find('a[href*="/bqg/"]').first();
      }
      ...
      const latest = clean(root2.find(".update a").first().text());
      const description = clean(root2.find(".review").first().text());
...
function summary(url, title, author, coverUrl, description, latest, categoryValues, status) {
  return Object.freeze({
    ...
    publishedAt: null,
    updatedAt: null,
    latestChapter: latest === null ? null : Object.freeze({
      id: null,
      title: latest,
      url: null,
      updatedAt: null
...
function parseChapterText(html3) {
  ...
  $("#chaptercontent p").each((_, element) => {
    ...
    if (line !== null && !/(?:加入书签|章节报错|本章未完|点击下一页)/u.test(line)) {
      lines.push(line);
    }
  });
  return lines.join("\n\n");
...
function isChapterUrl(url) {
  return url.origin === origin && /^\/bqg\/\d+\/\d+(?:_\d+)?\.html$/u.test(url.pathname);
}
