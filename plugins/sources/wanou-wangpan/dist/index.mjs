...
var videoExtensions = /* @__PURE__ */ new Set(["mp4", "mkv", "avi", "mov", "wmv", "flv", "webm", "m4v", "ts"]);
var context;
...
async function getContent(request) {
  ...
  return frozen({
    ...
    pages: [],
    media: {
      url: requireContext().resource.proxy({ kind: resourceType, url: resolved.url, headers }),
      resourceType,
...
async function openProviderLogin(provider) {
  ...
  await withPage(async (page) => {
    await page.navigate(url, { timeoutMs: 45e3 });
...
async function listUcLikeShare(share) {
  ...
  return withPage(async (page) => {
    await page.navigate(share.url, { timeoutMs: 45e3 });
    const token = await page.fetch({ url: `${origin}/1/clouddrive/share/sharepage/token?${query}`, method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ pwd_id: share.id, passcode: share.passcode }), responseType: "json", timeoutMs: 3e4 });
    ...
    while (queue.length > 0 && visited.size < 30) {
      ...
      if (folder === void 0 || visited.has(folder.id)) continue;
      ...
      const response = await page.fetch({ url, responseType: "json", timeoutMs: 3e4 });
...
async function listBaiduShare(share) {
  return withPage(async (page) => {
    ...
    const files = list.flatMap((value) => {
      ...
      if (id === "" || name === "" || !isVideo(name, value)) return [];
...
async function resolvePlayback(chapter) {
  return withPage(async (page) => {
    await page.navigate(chapter.share.url, { timeoutMs: 45e3 });
...
async function fetchSite(url) {
  ...
  if (response.status === 403 || response.status === 429) {
    ...
    return withPage(async (page) => {
      await page.navigate(url, { timeoutMs: 45e3 });
      return page.getHtml({ timeoutMs: 2e4 });
    });
  }
...
function collectionResponse(id, title, request, values, page) {
  ...
  if (request.collectionId !== null) {
    if (request.collectionId !== id) throw new Error("Discovery collection is invalid.");
...
function blocksByClass(html, className) {
  const starts = [...html.matchAll(/<div\b[^>]*class=["']([^"']+)["'][^>]*>/giu)].filter((match) => match[1]?.split(/\s+/u).includes(className));
...
function summary(value) {
  return frozen({ id: `wanou:${value.id}`, title: value.title, contentKind: "video", coverOrientation: "portrait", author: null, url: value.url, coverUrl: value.cover === "" ? null : requireContext().resource.proxy({ kind: "image", url: value.cover, headers: { Referer: `${site}/` } }), description: value.latest === "" ? null : value.latest, language: "zh-CN", status: "unknown", access: "unknown", wordCount: null, chapterCount: null, publishedAt: null, updatedAt: null, latestChapter: null, categories: [], tags: [], attributes: [] });
}
function chapterItem(share, file, order) {
  const id = encodeText(JSON.stringify({ share, file }));
  return frozen({ id: `wanou-chapter:${id}`, title: file.name, order, url: null, volumeTitle: providerLabel(share.provider), wordCount: null, updatedAt: null, isLocked: file.id === "login", attributes: [] });
}
function decodeChapter(value) {
  const encoded = /^wanou-chapter:([A-Za-z0-9_-]+)$/u.exec(value)?.[1];
  if (encoded === void 0) throw new Error("Chapter ID is invalid.");
  try {
    const parsed = JSON.parse(decodeText(encoded));
    if (!isRecord(parsed) || !isRecord(parsed.share) || !isRecord(parsed.file)) throw new Error();
    const provider = parsed.share.provider;
    if (provider !== "baidu" && provider !== "quark" && provider !== "uc") throw new Error();
    ...
    if (url === "" || id === "" || name === "" || fileId === "") throw new Error();
...
function contentUrl(value) {
  const encoded = /^wanou:([A-Za-z0-9_-]+)$/u.exec(value)?.[1];
  if (encoded === void 0) throw new Error("Content ID is invalid.");
  const url = safeUrl(decodeText(encoded));
  if (!url.startsWith(site)) throw new Error("Content ID is invalid.");
  return url;
}
function categoryUrl(id, page) {
  return page === 1 ? `${site}/vodshow/${id}-----------.html` : `${site}/vodshow/${id}--------${page}---.html`;
}
function withPage(action) {
  const run = pageQueue.then(async () => action(await requireContext().webview.open({ visible: false, timeoutMs: 3e4 })));
  pageQueue = run.then(() => void 0, () => void 0);
  return run;
}
function providerLabel(provider) {
  return provider === "quark" ? "夸克网盘" : provider === "uc" ? "优汐网盘" : "百度网盘";
}
function loginHint(provider) {
  return `${providerLabel(provider)}（请先在发现页的“网盘登录”完成登录）`;
}
function isVideo(name, value) {
  const extension = /\.([a-z0-9]+)(?:$|[?#])/iu.exec(name)?.[1]?.toLowerCase() ?? "";
  return videoExtensions.has(extension) || /video/iu.test(firstString(value.obj_category, value.category, value.file_type));
}
function nestedArray(value, keys) {
  let current = value;
...
function firstArray(value, ...keys) {
  ...
  for (const key of keys) {
    ...
    if (isRecord(candidate) && Array.isArray(candidate.list)) return candidate.list;
  }
  return [];
}
...
function number(...values) {
  const value = Number(values.find((item) => item !== null && item !== void 0 && item !== "") ?? 0);
...
function cursorPage(cursor, target) {
  ...
  const value = Number(cursor.startsWith(`${target}:`) ? cursor.slice(target.length + 1) : "");
  if (!Number.isSafeInteger(value) || value < 2 || value > 50) throw new Error("Cursor is invalid.");
  return value;
}
...
function attribute(value, pattern) {
  return pattern.exec(value)?.[1] ? decodeHtml(pattern.exec(value)?.[1] ?? "") : "";
}
...
function decodeHtml(value) {
  return value.replace(/&(?:amp|#38);/giu, "&").replace(/&nbsp;/giu, " ").replace(/&quot;/giu, '"').replace(/&#39;/giu, "'");
}
function clean(value) {
  return value.replace(/[\s\u3000\u00a0]+/gu, " ").trim();
}
function passwordNear(value, index) {
  return /(?:提取码|密码|pwd|passcode)\s*[:：=]\s*([A-Za-z0-9]{4,})/iu.exec(value.slice(Math.max(0, index - 120), index + 240))?.[1] ?? "";
}
