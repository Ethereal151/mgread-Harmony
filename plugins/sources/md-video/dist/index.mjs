...
async function discover(request) {
  if (request.target === null)
    return frozen({ kind: "document", document: { components: [{ type: "section", id: "md-video-channels", title: "MD视频", subtitle: "苹果 CMS 最新内容", icon: "video", children: [{ type: "categoryCollection", id: "md-video-channel-list", layout: "chips", categories: [{ id: "latest", title: "最新", target: "channel:latest", count: null, url: null, icon: "video" }] }] }] } });
  if (request.target !== "channel:latest")
...
async function getContent(request) {
  ...
  if (!episode || !safeUrl(episode.url))
...
async function api(params) {
  let last;
  for (const base of [active, ...mirrors.filter((value) => value !== active)])
    try {
      const url = new URL(`${base}/api.php/provide/vod/`);
      url.searchParams.set("ac", "detail");
      for (const [key, value2] of Object.entries(params))
        url.searchParams.set(key, String(value2));
