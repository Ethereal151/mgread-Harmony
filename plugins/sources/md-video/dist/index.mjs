...
// src/index.mts
var mirrors = ["http://mdav.pw", "http://md91.cc", "http://9191md.me", "http://36717.info", "http://36717.pw", "http://bt4.cc"];
var headers = { "User-Agent": "Mozilla/5.0", Accept: "application/json,text/plain,*/*" };
var context;
var active = mirrors[0];
...
async function search(request) {
  ...
  const page = cursorPage(request.cursor, "search"), data = await api({ wd: query, pg: page }), size = clamp(request.pageSize), items = records(data.list).map(summary).filter(notNull).slice(0, size);
  return frozen({ items, nextCursor: page < number(data.pagecount) && items.length ? `search:${page + 1}` : null, totalCount: number(data.total) || null });
}
async function searchSuggestions(_request) {
  return frozen({ items: [], nextCursor: null });
}
async function discover(request) {
  if (request.target === null) return frozen({ kind: "document", document: { components: [{ type: "section", id: "md-video-channels", title: "MD视频", subtitle: "苹果 CMS 最新内容", icon: "video", children: [{ type: "categoryCollection", id: "md-video-channel-list", layout: "chips", categories: [{ id: "latest", title: "最新", target: "channel:latest", count: null, url: null, icon: "video" }] }] }] } });
  if (request.target !== "channel:latest") throw new Error("Discovery target is invalid.");
  const page = cursorPage(request.cursor, request.target), data = await api({ pg: page }), size = clamp(request.pageSize), contents = records(data.list).map(summary).filter(notNull).slice(0, size), collectionId = "md-video:latest", items = contents.map((content) => frozen({ content, rank: null, metric: null, recommendation: null })), continuation = page < number(data.pagecount) && contents.length ? frozen({ target: request.target, cursor: `${request.target}:${page + 1}` }) : null;
...
async function getDetail(request) {
  const id = contentId(request.id), data = await detail(id), item = summary(data);
  if (!item) throw new Error("Video detail is unavailable.");
  const episodes = parseEpisodes(data);
  return frozen({ ...item, description: clean(text(data.vod_blurb || data.vod_content)) || null, chapterCount: episodes.length, latestChapter: episodes.length ? { id: episodeId(id, episodes.at(-1)), title: episodes.at(-1)?.title ?? "", url: null, updatedAt: null } : null, aliases: [], catalogUrl: `${active}/api.php/provide/vod/?ac=detail&ids=${id}` });
}
async function getChapters(request) {
  const id = contentId(request.id), episodes = parseEpisodes(await detail(id)), items = episodes.map((episode, order) => frozen({ id: episodeId(id, episode), title: episode.title, order, url: null, volumeTitle: episode.group, wordCount: null, updatedAt: null, isLocked: false, attributes: [] })), groups = [...new Set(episodes.map((value) => value.group))].map((title, index) => frozen({ id: `group:${id}:${index}`, title, order: index, episodes: items.filter((item) => item.volumeTitle === title) }));
  return frozen({ items, groups });
}
async function getContent(request) {
  const id = contentId(request.id), key = chapterKey(request.chapterId, id), episode = parseEpisodes(await detail(id)).find((value) => value.line === key.line && value.index === key.index);
  if (!episode || !safeUrl(episode.url)) throw new Error("Video address is unavailable.");
  const resourceType = /\.m3u8(?:$|[?#])/iu.test(episode.url) ? "hls" : "video", mediaHeaders = { Referer: `${active}/`, "User-Agent": headers["User-Agent"] };
...
async function detail(id) {
  const data = await api({ ids: id }), value = records(data.list)[0];
  if (!value) throw new Error("Video detail is unavailable.");
  return value;
}
async function api(params) {
  let last;
  for (const base of [active, ...mirrors.filter((value) => value !== active)]) try {
    const url = new URL(`${base}/api.php/provide/vod/`);
    url.searchParams.set("ac", "detail");
    for (const [key, value2] of Object.entries(params)) url.searchParams.set(key, String(value2));
    const response = await requireContext().http.fetch(url, { headers });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    const value = await response.json();
    if (!isObject(value) || number(value.code) !== 1) throw new Error("Invalid CMS response.");
    active = base;
    return value;
  } catch (error) {
    last = error;
...
function parseEpisodes(value) {
  const lines = text(value.vod_play_url).split("$$$"), names = text(value.vod_play_from).split("$$$"), episodes = [];
  for (const [line, raw] of lines.entries()) for (const [episodeIndex, entry] of raw.split("#").entries()) {
...
function chapterKey(id, content) {
  ...
  return { line: Number(match[1]), index: Number(match[2]) };
}
