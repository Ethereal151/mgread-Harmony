...
var signKey = "T^xS0x31XL%wEowC";
var commonHeaders = Object.freeze({
  ...
  if (request.target === null) {
    ...
    components.push({
      ...
      children: [{
        ...
        id: "midu-category-list",
        layout: "chips",
        categories: categories.map(([id, title2]) => ({ id, title: title2, target: `category:${id}`, count: null, url: null, icon: categoryIcon(id) }))
      }]
    });
...
async function getDetail(request) {
  ...
  if (projection === void 0) {
    ...
    projection = {
      ...
      category: "",
      tags: [],
      chapterCount: catalog.rows.length,
      status: "unknown"
    };
...
function projectBook(raw) {
  ...
  return frozen({
    id,
    title,
    author: clean(text(value.author)),
    cover: coverUrl(text(value.cover ?? value.coverUrl)),
    description: clean(text(value.description)) || clean(stripHtml(text(raw.emDescription ?? value.emDescription))),
    category: clean(text(value.category)),
    tags,
    chapterCount,
...
function summary(book) {
  ...
  return frozen({
    ...
    url: readerUrl(book.id),
    coverUrl: proxyImage(book.cover),
...
async function signedPost(path, specific) {
  ...
  const merged = {
    app: "Mdqtt.saas",
    version: "300",
    nonce: String(Math.floor(Math.random() * 9e3 + 1e3)),
    time: timestamp,
    headerQueryTime: timestamp,
    tuid: anonymousTuid || `${Date.now()}_Mdqtt.saas`,
    app_source: ""
  };
...
async function postForm(url, body, referer) {
  const raw = await fetchText(url, {
    method: "POST",
    headers: { ...commonHeaders, Referer: referer, "Content-Type": "application/x-www-form-urlencoded" },
    body: body.toString()
  });
