...
async function requireAccessible(page, url) {
  await page.navigate(url, { timeoutMs: 35e3 });
...
function htmlText(value) {
  return clean(value.replace(/<script\b[\s\S]*?<\/script>|<style\b[\s\S]*?<\/style>|<[^>]+>/giu, " ").replace(/&(nbsp|amp|quot|#39);/giu, (_all, entity) => ({ nbsp: " ", amp: "&", quot: '"', "#39": "'" })[entity.toLowerCase()] ?? " "));
}
function htmlAttribute(value, name) {
  return clean(new RegExp(`\\b${escapeRegex(name)}\\s*=\\s*(["'])(.*?)\\1`, "isu").exec(value)?.[2] ?? "");
}
function text(value) {
...
function isVerificationText(value) {
  return /安全验证|浏览器安全检查|recaptcha/iu.test(value);
}
