...
test('live source completes category, search, detail, catalog and content', { timeout: 60000 }, async (t) => {
  ...
  await plugin.activate({ dataDir: join(root, 'data'), cacheDir: join(root, 'cache'), http: { fetch }, errors: { raise(error) { throw Object.assign(new Error(error.message), { name: 'PluginManagerError', ...error }); } }, resource: { proxy: () => 'http://127.0.0.1:1234/v1/source-resource/opaque' }, log: { debug() {}, info() {}, warn() {}, error() {} }, app: { runtimeVersion: 'live', nodeVersion: process.versions.node, pluginApi: 1 }, plugin: { id: 'org.mgread.shudugu', version: '0.1.6' } });
