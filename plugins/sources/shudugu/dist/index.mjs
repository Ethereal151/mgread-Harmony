...
var discoveryDetailPolicy = Object.freeze({ namespace: "detail", staleAfterMs: 60 * 60 * 1e3, serveStaleWhileRevalidate: true });
var detailProjectionPolicy = Object.freeze({ namespace: "detail-projection-v1", staleAfterMs: 60 * 60 * 1e3, allowStaleOnError: false });
var discoveryDetailProjectionPolicy = Object.freeze({ namespace: "detail-projection-v1", staleAfterMs: 60 * 60 * 1e3, serveStaleWhileRevalidate: true });
var hotSearchPolicy = Object.freeze({ namespace: "hot-search", staleAfterMs: 24 * 60 * 60 * 1e3 });
...
var ShuduguSource = class {
    ...
    if (request.target === null) {
      ...
      if (home.latest.length !== 0) {
        components.push(Object.freeze({
          type: "section",
          id: "shudugu-latest-section",
          title: "正在热更",
          subtitle: "来自官网最新更新",
          icon: "ongoing",
          children: Object.freeze([Object.freeze({
            type: "contentCollection",
            id: "shudugu-latest-books",
            layout: "shelf",
            items: this.#discoveryItems(home.latest),
            continuation: null
          })])
        }));
      }
      ...
      if (home.ranked.length !== 0) {
        overviewChildren.push(Object.freeze({
          type: "section",
          id: "shudugu-ranking-section",
          title: "阅读排行",
          subtitle: "站内热门作品",
          icon: "ranking",
          children: Object.freeze([Object.freeze({
            type: "contentCollection",
            id: "shudugu-ranking-books",
            layout: "compact",
            items: this.#discoveryItems(home.ranked, true),
            continuation: null
          })])
        }));
      }
      if (home.completed.length !== 0) {
        overviewChildren.push(Object.freeze({
          type: "section",
          id: "shudugu-completed-section",
          title: "完结精选",
          subtitle: "一次读到结局",
          icon: "completed",
          children: Object.freeze([Object.freeze({
            type: "contentCollection",
            id: "shudugu-completed-books",
            layout: "coverGrid",
            items: this.#discoveryItems(home.completed),
            continuation: null
          })])
        }));
      }
      ...
      components.push(Object.freeze({
        type: "section",
        id: "shudugu-categories-section",
        title: "探索分类",
        subtitle: "按题材继续发现",
        icon: "explore",
        children: Object.freeze([Object.freeze({
          type: "categoryCollection",
          id: "shudugu-categories",
          layout: "chips",
          categories: Object.freeze(this.#categories.map((category2) => Object.freeze({
            id: `category:${category2.id}`,
            title: category2.title,
            target: `category:${category2.id}`,
            count: null,
            icon: categoryIcon(category2.id, category2.title),
            url: new URL(`/${category2.id}/`, this.#baseUrl).toString()
          })))
        })])
      }));
  ...
  async #loadDetailProjection(request, cachePolicy) {
    ...
    const latestChapter = latestTitle === null || latestHref === void 0 ? null : Object.freeze({
      id: this.#chapterId(this.#sourceUrl(latestHref, url)),
      title: latestTitle,
      url: this.#sourceUrl(latestHref, url).toString(),
      updatedAt
    });
  ...
  async #getHtmlResult(url, policy) {
    ...
    return policy === void 0 ? Object.freeze({ body: await request(), storedAtMs: Date.now() }) : this.#cache.getOrFetchTextResult(url, policy, request).then((result) => Object.freeze({ body: result.value, storedAtMs: result.storedAtMs }));
  }
  ...
  async #loadHomeDiscovery() {
    try {
      ...
      const html3 = await this.#getHtml(homeUrl, discoveryListingPolicy);
      const latest = (await this.#parseList(html3, homeUrl)).slice(0, 10);
      const ranked = (await this.#parseHomeLinks(html3, homeUrl, "阅读排行", "ongoing")).slice(0, 8);
      const completedLinks = (await this.#parseHomeLinks(html3, homeUrl, "完结小说", "completed")).slice(0, 6);
  ...
  async #parseHomeLinks(html3, pageUrl, heading, status) {
    const cheerio = await loadCheerio();
    const $ = cheerio.load(html3);
  ...
  #discoveryItems(contents2, ranked = false) {
    return Object.freeze(contents2.map((content, index2) => Object.freeze({
      content,
      rank: ranked ? index2 + 1 : null,
      metric: null,
      recommendation: null
    })));
  }
  ...
  #parseBook($, element, pageUrl, seen) {
    ...
    const latest = root2.find(".itemtxt ul li a").last();
    const latestTitle = textOrNull(latest.text());
    const latestHref = latest.attr("href");
    const latestChapter = latestTitle === null || latestHref === void 0 ? null : Object.freeze({ id: this.#chapterId(this.#sourceUrl(latestHref, pageUrl)), title: latestTitle, url: this.#sourceUrl(latestHref, pageUrl).toString(), updatedAt: null });
    return [this.#summary({ id, title, author, category: textOrNull(spans[1]), coverUrl: this.#proxyCoverUrl(root2.find("img").first().attr("src"), pageUrl), description: null, status: parseStatus(spans[0]), wordCount: null, chapterCount: null, latestChapter, updatedAt: null })];
  }
  #summary(input) {
  ...
  #nextCatalogPage($, current, bookId) {
    const next2 = $('.pages a[href], a[rel="next"]').toArray().find((element) => {
  ...
  #nextChapterPage($, current, firstPage) {
    const next2 = $('.prenext a[href], a[rel="next"]').toArray().find((element) => {
      const label = (textOrNull($(element).text()) ?? "").replace(/\s+/gu, "");
  ...
  #categoryUrl(id, page) {
    return new URL(page === 1 ? `/${id}/` : `/${id}/${page}.html`, this.#baseUrl);
  }
...
function documentResult(section) {
  return Object.freeze({ kind: "document", document: Object.freeze({ components: Object.freeze([section]) }) });
}
...
function novelIdFromUrl(url) {
  return /^\/(\d+)\/$/u.exec(url.pathname)?.[1] ?? null;
}
...
function parseSearchTotal(value) {
  const count = Number(/共(\d+)本小说/u.exec(value.replace(/\s+/g, ""))?.[1] ?? Number.NaN);
...
async function discover(request) {
  return invoke("discover", () => requireSource().discover(request));
}
async function search(request) {
  return invoke("search", () => requireSource().search(request));
}
async function searchSuggestions(request) {
  return invoke("search_suggestions", () => requireSource().searchSuggestions(request));
}
async function getDetail(request) {
  return invoke("get_detail", () => requireSource().getDetail(request));
}
async function getChapters(request) {
  return invoke("get_chapters", () => requireSource().getChapters(request));
}
