...
var agent = "okhttp/4.10.0";
var baseHeaders = { "User-Agent": agent, platform: "1", version_name: "3.9.2" };
var context;
var tokenPromise;
var channelsPromise;
...
async function getDetail(request) {
  const id = contentId(request.id), data = await detail(id), item = summary(data, id);
  return frozen({ ...item, aliases: [], catalogUrl: `${root}v2/theater_parent/detail?theater_parent_id=${encodeURIComponent(id)}`, chapterCount: records(data.theaters).length });
}
async function getChapters(request) {
  const id = contentId(request.id), episodes = records((await detail(id)).theaters), items = episodes.map((value, index) => chapter(id, value, index));
  return frozen({ items, groups: items.length ? [frozen({ id: `group:${id}:main`, title: "默认线路", order: 0, episodes: items })] : [] });
}
...
async function getJson(path) {
  const response = await requireContext().http.fetch(new URL(path, root).toString(), { headers: await authorizedHeaders() });
  return parseResponse(response);
}
async function postJson(path, body) {
  const response = await requireContext().http.fetch(new URL(path, root).toString(), { method: "POST", headers: { ...await authorizedHeaders(), "Content-Type": "application/json" }, body: JSON.stringify(body) });
  return parseResponse(response);
}
...
function summary(value, id) {
  const finished = value.finish === true || Number(value.finish) === 1, total = nonNegative(value.total);
  return frozen({ id: `theater:${id}`, title: text(value.title) || id, contentKind: "video", coverOrientation: "portrait", author: null, url: `${root}v2/theater_parent/detail?theater_parent_id=${encodeURIComponent(id)}`, coverUrl: proxyImage(text(value.cover_url)), description: nullable(first(value.introduction, value.description)), language: "zh-CN", status: finished ? "completed" : "ongoing", access: "unknown", wordCount: null, chapterCount: total, publishedAt: null, updatedAt: null, latestChapter: total === null ? null : { id: `theater:${id}:latest`, title: `共${total}集`, url: null, updatedAt: null }, categories: stringList(value.desc_tags), tags: [], attributes: [] });
}
function chapter(id, value, index) {
  const native = episodeId(value, index), number = text(first(value.num, value.episode_num)) || String(index + 1);
  return frozen({ id: `theater:${id}:${native}`, title: `第${number}集`, order: index, url: null, volumeTitle: "默认线路", wordCount: null, updatedAt: null, isLocked: false, attributes: [] });
}
function episodeId(value, index) {
  const id = text(first(value.id, value.theater_id, value.num, value.episode_num));
  return /^[\w-]+$/u.test(id) ? id : String(index + 1);
}
