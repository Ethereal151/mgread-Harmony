...
var categories = Object.freeze([
  ...
  ["mystery", "悬疑", "sort", "7"],
  ["game", "游戏", "sort", "8"],
  ["science-fiction", "科幻", "sort", "9"],
  ["sports", "体育", "sort", "10"],
  ["ancient-romance", "古言", "sort", "11"],
  ["modern-romance", "现言", "sort", "12"],
  ["fantasy-romance", "幻言", "sort", "13"],
  ["xianxia", "仙侠", "sort", "14"],
  ["youth", "青春", "sort", "15"],
  ["transmigration", "穿越", "sort", "16"],
  ["women", "女生", "sort", "17"],
  ["other", "其他", "sort", "18"],
  ["visits", "点击榜", "ranking", "allvisit"],
  ["votes", "推荐榜", "ranking", "allvote"],
  ["favorites", "收藏榜", "ranking", "goodnum"],
  ["new", "新书入库", "ranking", "postdate"]
]);
...
var TianyueSource = class {
  ...
  async #loadDiscovery(category, page, signal) {
    ...
    const init = { headers: { accept: "text/html,application/xhtml+xml", "accept-language": "zh-CN,zh;q=0.9", referer: `${origin}/` }, ...signal === void 0 ? {} : { signal } };
    const html3 = await this.#fetch(url, init);
    const $ = load(html3);
    const items = this.parseList(html3, url, category[1]);
  ...
  parseList(html3, pageUrl, fallbackCategory) {
    ...
    $("#alistbox").each((_, element) => {
      const root2 = $(element);
      const titleLink = root2.find(".info .title h2 a[href]").first();
  ...
  async #fallbackSearch(query) {
    ...
    const candidates = [...newest === void 0 ? [] : [newest], ...categories.filter(([id]) => id !== "new")].slice(0, searchFallbackPolicy.categoryBudget);
  ...
  #proxyImage(url, referer) {
    return url.protocol === "https:" && url.hostname === "img.xtyxsw.org" && referer.origin === origin ? this.context.resource.proxy({ kind: "image", url: url.toString(), headers: { Accept: "image/*", Referer: referer.toString() } }) : null;
  }
};
function summary(input) {
  const chapter = input.latestUrl === null ? null : chapterNumber(input.latestUrl, input.bookId);
...
function readUrl(bookId) {
  return new URL(`/read/${bookId}/`, origin);
}
function bookNumber(url) {
  return /^\/(?:book|read)\/(\d+)(?:\.html|\/)?$/u.exec(url.pathname)?.[1] ?? null;
}
function chapterNumber(url, bookId) {
  return new RegExp(`^/read/${bookId}/(\\d+)\\.html$`, "u").exec(url.pathname)?.[1] ?? null;
}
...
function isChapterContinuation(url, bookId, chapterId) {
  return url.origin === origin && new RegExp(`^/read/${bookId}/${chapterId}(?:_\\d+)?\\.html$`, "u").test(url.pathname);
}
