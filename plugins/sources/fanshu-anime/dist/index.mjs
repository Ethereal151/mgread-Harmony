...
var apiPath = "/yoapp.php";
var token = "yoapp_a682c34e5cc0e0c38b4f749475074db7281791ad";
var bootstrapSalt = "8124fb976064d07a5c6af58c771f1c87";
var fallbackAppSignature = "1F83BDCA0957AADDCD3CE53088D60FD228ADC463EBA105495E4EBAE6962B54D6";
var clientDeviceId = "do3bef9a-d716-4eed-8ed3-d6de71283f36";
var userAgent = "Dart/3.11 (dart:io)";
var playerUserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/136.0.0.0 Safari/537.36";
var preferredSources = Object.freeze(["mui6", "BY1", "cycp", "new_mui1", "new_mui2", "mui2", "BY2"]);
...
var searchTypes = Object.freeze(["1", "22", "3", "20", "21"]);
var context;
var auth;
var authPromise;
...
async function search(request) {
  ...
  const settled = await Promise.allSettled(searchTypes.map((typeId) => apiGet("category_videos", {
    type_id: typeId,
    page: String(page),
    page_size: String(Math.max(12, limit)),
    sort: "latest"
  })));
...
async function discover(request) {
  if (request.target === null) {
    ...
    components.push({
      type: "section",
      id: "fanshu-categories",
      title: "动漫分类",
      subtitle: "按栏目继续发现",
      icon: "video",
      children: [{
        type: "categoryCollection",
        id: "fanshu-category-list",
        layout: "chips",
...
async function getContent(request) {
  ...
  const candidates = selectedSource === void 0 ? sources : [selectedSource, ...sources.filter((source) => source !== selectedSource)];
...
function matchingEpisode(source, selected) {
  ...
  return episodes.find((episode) => text(episode.episode_id ?? episode.id) === selected.episodeId) ?? episodes.find((episode, index) => (text(episode.episode_index ?? episode.index) || String(index + 1)) === selected.episodeIndex) ?? null;
}
...
function incompatiblePlaylist(url) {
  ...
  return lower.includes("psch=v2") || lower.includes("pkey=") || lower.includes("playlisttype=lowlatency") || lower.includes(".m3u8") && (lower.includes("bytegooty.com/obj/") || lower.includes("lf3-cdn-tos.bytegooty.com"));
}
...
function searchable(value) {
  return [value.vod_name, value.name, value.title, value.vod_actor, value.vod_director, value.vod_class].map(text).join(" ").toLocaleLowerCase("zh-CN");
}
