...
async function getDetail(request) {
  const id = contentId(request.id), value = await detail(id);
  return frozen({ ...summary(value, id), aliases: [], catalogUrl: `${root}detail?id=${encodeURIComponent(id)}` });
}
async function getChapters(request) {
  const id = contentId(request.id), value = await detail(id), episodes = split(text(first(value.vod_play_url, value.play_url))), items = episodes.map((episode, index) => frozen({ id: `video:${id}:${index}`, title: episode.title, order: index, url: null, volumeTitle: "视频", wordCount: null, updatedAt: null, isLocked: false, attributes: [] }));
  return frozen({ items, groups: items.length ? [frozen({ id: `group:${id}:main`, title: "视频", order: 0, episodes: items })] : [] });
}
...
function derive(token) {
  const bytes = Buffer.from(token.slice(4), "hex"), mask = [104, 64, 70, 166, 190, 168, 143, 130, 225, 254, 251, 217, 196, 34, 45, 60, 29, 20, 103, 105], out = Buffer.alloc(bytes.length);
...
function summary(value, id) {
  const title = text(first(value.vod_name, value.title)) || id, remark = nullable(first(value.vod_remarks, value.remark));
  return frozen({ id: `video:${id}`, title, contentKind: "video", coverOrientation: "portrait", author: null, url: `${root}detail?id=${encodeURIComponent(id)}`, coverUrl: proxyImage(text(first(value.vod_pic, value.cover))), description: nullable(first(value.vod_content, value.description, remark)), language: "zh-CN", status: "unknown", access: "unknown", wordCount: null, chapterCount: countHint(remark), publishedAt: null, updatedAt: null, latestChapter: null, categories: stringList(value.type_name), tags: stringList(value.tags), attributes: [] });
}
function split(raw) {
  return raw.split("$$$").flatMap((group) => group.split("#")).map((row, index) => {
    const at = row.indexOf("$");
    return { title: at >= 0 ? row.slice(0, at) || `第 ${index + 1} 集` : `第 ${index + 1} 集`, url: at >= 0 ? row.slice(at + 1) : row };
...
function proxyImage(url) {
  return safeUrl(url) ? requireContext().resource.proxy({ kind: "image", url, headers: { Referer: "https://api.999888456.xyz/" } }) : null;
}
...
function countHint(value) {
  const match = value?.match(/(\d+)\s*集/u)?.[1], count = Number(match);
...
function first(...values) {
  return values.find((v) => v !== null && v !== void 0 && v !== "") ?? "";
}
function text(v) {
