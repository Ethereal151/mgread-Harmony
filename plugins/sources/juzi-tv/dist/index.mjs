...
async function getDetail(request) {
  const id = contentId(request.id), data = object((await post("/v2/api/vodInfo/index", { vodId: Number(id) })).data), item = summary(data, id);
...
async function getContent(request) {
  const id = contentId(request.id), epId = chapterNative(request.chapterId, id), options = records((await post("/v2/api/vodInfo/epDetail", { vodEpId: Number(epId) })).data), resolutions = [...new Set([...options.filter((value) => value.canPlay !== false).map((value) => Number(value.vodResolution)), 3, 2, 1].filter((value) => Number.isSafeInteger(value) && value > 0))];
  let upstream = "";
...
function sign(value) {
  ...
  return { ...value, sign: createHash("md5").update(canonical).digest("hex") };
}
...
function episodes(data) {
  const result = [];
  for (const [playerIndex, player] of records(data.playerList).entries()) {
