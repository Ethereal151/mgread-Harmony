...
// src/index.mts
var base = "https://hsck123.25img.com";
var searchKey = "ndafeoafa";
var headers = {
...
var context;
var detailLoads = /* @__PURE__ */ new Map();
...
async function discover(request) {
  ...
  const continuation = page.nextCursor === null ? null : frozen({ target: request.target, cursor: page.nextCursor });
...
function parseList(html) {
  ...
  for (const match of html.matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a>/giu)) {
    ...
    const cover = attribute(attributes, "data-original") || attribute(attributes, "data-src") || attribute(image, "data-original") || attribute(image, "data-src") || attribute(image, "src");
    const candidate = summary(id, title, cover === "" ? null : cover, null, null, base + "/");
    const previous = unique.get(id);
    if (previous === void 0 || previous.coverUrl === null && candidate.coverUrl !== null) unique.set(id, candidate);
  }
  return [...unique.values()];
}
function parseDetail(html, id) {
  const headings = [...html.matchAll(/<h3\b[^>]*class=["'][^"']*\btitle\b[^"']*["'][^>]*>([\s\S]*?)<\/h3>/giu)].map((match) => strip(match[1] ?? "")).filter((value) => value !== "" && value !== "目录" && value !== "为你推荐");
...
function summary(id, title, cover, updatedAt, chapterCount, coverReferer = base + "/") {
...
function categoryCounts(html) {
  ...
  for (const match of html.matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a>/giu)) {
    ...
    if (code === void 0 || countText === void 0) continue;
...
function parseCursor(cursor, prefix) {
  ...
  if (!Number.isSafeInteger(page) || page < 1 || page > 1e4 || !Number.isSafeInteger(offset) || offset < 0 || offset > 1e3) {
...
function totalPages(html) {
  ...
  return Number.isSafeInteger(value) && value > 0 ? Math.min(value, 1e4) : 1;
}
