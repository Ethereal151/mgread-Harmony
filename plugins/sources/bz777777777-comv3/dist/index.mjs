...
var PluginCache = class {
  ...
  getOrFetchJson(cacheKeyInput, policy, fetcher, decode2) {
    return this.getOrFetchJsonResult(cacheKeyInput, policy, fetcher, decode2).then((result) => result.value);
  }
  getOrFetchJsonResult(cacheKeyInput, policy, fetcher, decode2) {
    if (typeof cacheKeyInput !== "string" || cacheKeyInput.length === 0 || cacheKeyInput.length > 512) throw new Error("Cache key is invalid.");
    return this.#getOrFetch(cacheKey(policy.namespace, cacheKeyInput), policy, fetcher, decode2);
  }
  #getOrFetch(key, policy, fetcher, decode2) {
    ...
    const pending = this.#readOrFetch(key, policy, fetcher, decode2);
    this.#inflight.set(key, pending);
    void pending.then(() => this.#inflight.delete(key), () => this.#inflight.delete(key));
    return pending;
  }
  async #readOrFetch(key, policy, fetcher, decode2) {
    const cached = await this.#read(key, decode2);
    ...
    if (backgroundRefresh !== void 0) {
      ...
      const refreshed = await this.#read(key, decode2);
  ...
  async #read(key, decode2) {
    try {
      ...
      const value = decode2(record.value);
...
var BzSource = class {
  ...
  async search(query) {
    ...
    for (const path of urls) {
      ...
      const items = this.parseList(await this.#browser(url2, "GET", null), url2);
  ...
  parseList(html3, base) {
    ...
    $(".book-all-list li.column-2,.column-list ul.list li,.bookbox,.result-item,.book-img-text li,.rank-list li,.list-group-item,ul.searchlist li").each((_, element) => {
      const root2 = $(element);
      const link = root2.find('a.name,.name a,.right h4 a,.bookname a,h4 a,h3 a,a.book-name,.title a,a[href*="/book/"]').first();
      ...
      const authorText = clean(root2.find("a.author,.author,dt,.s4").first().text());
      const author = clean(/作者[：:]?\s*(.+)$/u.exec(authorText ?? "")?.[1]) ?? authorText;
      const kind = clean(root2.find(".cat,.s1").first().text());
      const latest = clean(root2.find("p.update a,.update a,.s3 a").first().text());
      items.push(summary(url, title, author, coverRaw === void 0 ? null : this.#proxy(new URL(coverRaw, base), base), null, latest, kind === null ? [] : [kind]));
    });
    return Object.freeze(items);
  }
  async getDetail(id) {
    const url = decode(id, "book");
    const html3 = await this.#cache.getOrFetchText(url, detail, () => this.#browser(url, "GET", null));
    const $ = load(html3);
  ...
  async chapters(id) {
    const first2 = decode(id, "book");
    const values = [];
    const seen = /* @__PURE__ */ new Set();
    let url = first2;
    for (let page = 0; page < 60; page += 1) {
      const html3 = await this.#cache.getOrFetchText(url, catalog, () => this.#browser(url, "GET", null));
      const $ = load(html3);
      ...
      for (const element of links) {
        ...
        if (!isChapter(chapter) || seen.has(chapter.toString())) continue;
      ...
      const next2 = $("a").filter((_, element) => clean($(element).text()) === "下一页").first().attr("href");
      if (next2 === void 0) break;
      const candidate = new URL(next2, url);
      if (candidate.origin !== origin || candidate.toString() === url.toString()) break;
      url = candidate;
    }
    if (values.length === 0) throw new Error("Catalog is empty.");
  ...
  async content(id, chapterId) {
    ...
    for (let page = 0; page < 15 && !seen.has(url.toString()); page += 1) {
      seen.add(url.toString());
      const html3 = await this.#browser(url, "GET", null);
      const $ = load(html3);
      const root2 = $(".page-content,#chaptercontent,#content,.read-content,#chapterText,.chapter-content").first();
      for (const paragraph of root2.find("p").length > 0 ? root2.find("p").toArray() : [root2.get(0)].filter(Boolean)) {
        const line = clean($(paragraph).text());
        if (line !== null && !/(?:本章未完|加入书签|章节报错)/u.test(line)) lines.push(line);
      }
      const next2 = $("a").filter((_, element) => clean($(element).text()) === "下一页").first().attr("href");
      if (next2 === void 0) break;
      const candidate = new URL(next2, url);
  ...
  #browser(url, method, body) {
    const operation = this.#browserTail.then(() => this.#browserNow(url, method, body));
    this.#browserTail = operation.then(() => void 0, () => void 0);
    return operation;
  }
  ...
  async #verify(page, url) {
    ...
    if (isChallenge(html3)) {
      ...
      if (isChallenge(html3)) this.#raiseAccessBlocked();
  ...
  async #fetchWithDiagnostics(page, url, method, body, attempt) {
    ...
    const response = await this.#fetch(page, url, method, body);
    this.context.log.info(`source_browser_${attempt}_fetch_completed_${statusClass(response.status)}`);
    return response;
  }
...
function decode(value, prefix) {
  ...
  if (prefix === "book" ? !isBook(url) : !isChapter(url)) throw new Error("Opaque ID is invalid.");
  return url;
}
function isBook(url) {
  return url.origin === origin && (/\/book\/\d+\/?$/u.test(url.pathname) || /\/\d+\/\d+\/?$/u.test(url.pathname));
}
function isChapter(url) {
  return url.origin === origin && (/\/book\/\d+\/\d+\.html$/u.test(url.pathname) || /\/(?:xs|look)\/\d+\/\d+\.html$/u.test(url.pathname));
}
...
function needsVerification(status, body) {
  return status === 403 || status === 503 || typeof body === "string" && isChallenge(body);
}
...
async function discover(request) {
  return invoke("discover", async () => {
    ...
    if (match?.[1] === void 0) throw new Error("Target invalid.");
    const page = cursor(request.cursor, `category:${match[1]}`);
    const items = (await req().discover(match[1], page)).slice(0, request.pageSize).map((content) => Object.freeze({ content, rank: null, metric: null, recommendation: null }));
...
async function getDetail(request) {
  return invoke("detail", () => req().getDetail(request.id));
}
async function getChapters(request) {
  return invoke("chapters", () => req().chapters(request.id));
}
async function getContent(request) {
  return invoke("content", () => req().content(request.id, request.chapterId));
}
function req() {
  if (context === void 0) throw new Error("Source not activated.");
...
function cursor(value, scope) {
  ...
  if (!Number.isSafeInteger(page) || page < 2) throw new Error("Cursor invalid.");
  return page;
}
export {
  activate,
