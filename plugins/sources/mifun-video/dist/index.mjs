...
var channels = [{ id: "0", title: "全部" }, { id: "1", title: "番剧" }, { id: "2", title: "国创" }, { id: "3", title: "剧场" }, { id: "4", title: "美漫" }, { id: "5", title: "特摄" }];
var context;
var base = initial;
var basePromise;
...
function detail(source, id) {
  return { id, title: clean(pick(source, /<h1[^>]*>([\s\S]*?)<\/h1>/u)) || id, cover: absolute(pick(source, /<div\b[^>]*class="[^"]*hl-dc-pic[^"]*"[\s\S]*?data-original="([^"]+)"/u) || pick(source, /property="og:image" content="([^"]+)"/u)), author: clean(pick(source, /导演：<\/em>([\s\S]*?)<\/li>/u) || pick(source, /主演：<\/em>([\s\S]*?)<\/li>/u)), remark: clean(pick(source, /class="[^"]*hl-content-text[^"]*"[^>]*>([\s\S]*?)<\/span>/u) || pick(source, /<meta name="description" content="([^"]*)"/u)), category: clean(pick(source, /类型：<\/em>([\s\S]*?)<\/li>/u)) };
}
...
function hasLink(source, path) {
  return new RegExp(`href=["']${path.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")}["']`, "u").test(source);
}
function pick(value, pattern) {
  return pattern.exec(value)?.[1] ?? "";
}
function clean(value) {
  return decode(value.replaceAll(/<[^>]+>/gu, " ").replaceAll("&nbsp;", " ").replaceAll(/\s+/gu, " ").trim());
}
function decode(value) {
  return value.replaceAll(/\\u([0-9a-fA-F]{4})/gu, (_all, hex) => String.fromCharCode(Number.parseInt(hex, 16))).replaceAll("\\/", "/").replaceAll('\\"', '"').replaceAll("&amp;", "&");
}
