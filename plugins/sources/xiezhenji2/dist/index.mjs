...
var filters = {
  contains(next2, text4, { adapter }) {
    return function contains2(elem) {
      return next2(elem) && adapter.getText(elem).includes(text4);
    };
  },
  icontains(next2, text4, { adapter }) {
...
var categories = Object.freeze([
  ["home", "首页", "/"],
  ["video", "Video Cosplay", "/category/video-cosplayyy/"],
  ["nude", "Cosplay Nude", "/category/cosplay-nudee/"],
  ["ero", "Cosplay Ero", "/category/cosplay-ero/"],
  ["cosplay", "Cosplay", "/category/cosplay/"],
  ["day", "24 Hours", "/24-hours/"],
  ["three-day", "3 Day", "/3-day/"],
  ["week", "7 Day", "/7-day/"],
...
var Xiezhenji2Source = class {
  ...
  async getDetail(id) {
    ...
    const author = text3($('.entry-content blockquote a[href*="/category/"]').first().text());
  ...
  parseList(html3, pageUrl) {
    ...
    $("#post-list .post-item, main .post-item, .post-item").each((_, element) => {
      ...
      items.push(summary(encodeId(url), title, url, cover, text3(root2.find(".from_the_blog_excerpt").text()), null, unique([root2.find(".cat-label").text()])));
    });
    return Object.freeze(items);
  }
  async #cachedHtml(url, policy) {
...
function summary(id, title, url, coverUrl, description, author, tags) {
...
function isPostUrl(url) {
  return url.origin === origin && /^\/[^/?#]+\/?$/u.test(url.pathname) && !/^\/(?:category|tag|author|page|wp-json|wp-content)\//u.test(url.pathname);
}
...
function srcsetUrls(value) {
  return value === void 0 ? Object.freeze([]) : Object.freeze(value.split(",").map((candidate) => candidate.trim().split(/\s+/u)[0]).filter((candidate) => candidate !== void 0 && candidate !== ""));
}
function parseCount(value) {
  const match = /(\d+)\s+photos?/iu.exec(value);
  const count = Number(match?.[1]);
  return Number.isSafeInteger(count) && count >= 0 ? count : null;
}
function collectPageUrls(html3, base) {
  const $ = load(html3);
...
function parseImages(html3, base) {
  const $ = load(html3);
  const links = $(".entry-content .gallery a[href]").toArray().flatMap((element) => {
    const href = $(element).attr("href");
    return href === void 0 ? [] : [new URL(href, base)];
  });
  const images = links.length === 0 ? $(".entry-content .gallery img, .entry-content img.attachment-full").toArray().flatMap((element) => {
    const raw = $(element).attr("data-src") ?? $(element).attr("data-lazy-src") ?? $(element).attr("src");
    return raw === void 0 ? [] : [new URL(raw, base)];
  }) : links;
  return uniqueUrls(images.filter((url) => url.hostname === "cosplaytele.com" && /\/wp-content\/uploads\//u.test(url.pathname) && /\.(?:jpe?g|png|webp|gif)$/iu.test(url.pathname) && !url.pathname.includes("cropped-icon-")));
}
function imageMime(url) {
  const extension = /\.([A-Za-z0-9]+)$/u.exec(url.pathname)?.[1]?.toLowerCase();
...
async function search(request) {
  return invoke("search", async (active) => {
    ...
    const items = (await active.search(request.query, page)).slice(0, request.pageSize);
...
async function getChapters(request) {
  return invoke("get_chapters", async (active) => active.getChapters(request.id));
}
