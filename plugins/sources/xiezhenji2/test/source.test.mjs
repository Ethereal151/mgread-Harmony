...
test('fixture chain covers discovery, search, detail, complete gallery and resource proxy', async (t) => {
  ...
  const proxied = []; const requested = [];
  await plugin.activate({ dataDir: cacheDir, cacheDir, app: { runtimeVersion: 'test', nodeVersion: process.versions.node, pluginApi: 1 }, plugin: { id: 'org.mgread.xiezhenji2', version: '1.0.4' }, log: { debug() {}, info() {}, warn() {}, error() {} }, resource: { proxy(request) { proxied.push(request); return `http://127.0.0.1/resource/${proxied.length}`; } }, http: { async fetch(input, init) { const headers = new Headers(init?.headers); assert.equal(headers.has('user-agent'), false); const url = new URL(input); requested.push(url); if (url.pathname.endsWith('/2/')) return new Response(second); if (url.pathname === '/fixture-post/') return new Response(detail); if (url.pathname.includes('/wp-content/')) return new Response(new Uint8Array([1, 2, 3]), { headers: { 'content-type': 'image/jpeg' } }); return new Response(list); } } });
  ...
  await plugin.discover({ target: 'category:video', cursor: null, collectionId: null, pageSize: 5 });
  await plugin.discover({ target: 'category:nude', cursor: null, collectionId: null, pageSize: 5 });
