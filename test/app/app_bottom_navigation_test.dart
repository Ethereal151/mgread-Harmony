...
void main() {
  testWidgets('shrinks, slides and expands the selected icon capsule', (WidgetTester tester) async {
    ...
    final Finder backdrop = find.byKey(const Key('app-bottom-navigation-backdrop'));
    final Finder indicator = find.byKey(const Key('app-bottom-navigation-moving-indicator'));
    final BoxDecoration backdropDecoration = tester.widget<DecoratedBox>(backdrop).decoration as BoxDecoration;
    ...
    expect(indicatorDecoration.color, AppThemeTokens.of(tester.element(indicator)).accentSoft);
    expect(initialIndicator.center.dx, closeTo(tester.getCenter(homeItem).dx, 0.1));
    expect(initialIndicator.center.dy, closeTo(tester.getRect(find.byKey(const Key('app-bottom-navigation'))).center.dy, 0.1));
    ...
    expect(tester.getSize(indicator).width, greaterThan(initialIndicator.width));
    ...
    expect(_iconScale(tester, AppNavigationDestination.search), AppMotion.bottomNavigationSelectedIconScale);

    await tester.pumpAndSettle();
    final Rect settledIndicator = tester.getRect(indicator);
    expect(settledIndicator.center.dx, closeTo(tester.getCenter(searchItem).dx, 0.1));
    expect(settledIndicator.width, closeTo(AppSpacing.bottomNavigationIndicatorWidth, 0.1));
    expect(settledIndicator.height, closeTo(AppSpacing.bottomNavigationIndicatorHeight, 0.1));

    final Finder profileItem = find.byKey(const Key('app-nav-profile'));
    await tester.tap(profileItem);
    await tester.pumpAndSettle();
    expect(tester.getRect(indicator).center.dx, closeTo(tester.getCenter(profileItem).dx, 0.1));
    expect(_labelOpacity(tester, AppNavigationDestination.profile), 0);
  });

  testWidgets('retargets rapid taps from the current capsule position', (WidgetTester tester) async {
    AppNavigationDestination selected = AppNavigationDestination.home;
    ...
    expect(selected, AppNavigationDestination.search);
    expect(tester.getRect(indicator).center.dx, closeTo(tester.getCenter(find.byKey(const Key('app-nav-search'))).dx, 0.1));
  });

  testWidgets('settles the capsule and implicit feedback immediately when reduced', (WidgetTester tester) async {
    AppNavigationDestination selected = AppNavigationDestination.home;
    ...
    expect(tester.getCenter(indicator).dx, closeTo(tester.getCenter(find.byKey(const Key('app-nav-discover'))).dx, 0.1));
    expect(tester.widget<AnimatedAlign>(find.byKey(const Key('app-nav-icon-motion-discover'))).duration, Duration.zero);
  });

  testWidgets('releases active navigation tickers when its scope is removed', (WidgetTester tester) async {
    await tester.pumpWidget(
      MaterialApp(
        theme: AppTheme.light(),
        home: AppBottomNavigationMotionScope(
          child: Scaffold(
            bottomNavigationBar: AppBottomNavigation(selected: AppNavigationDestination.home, onSelected: (_) {}),
          ),
        ),
      ),
    );
...
double _labelOpacity(WidgetTester tester, AppNavigationDestination destination) {
  return tester
      .widget<AnimatedOpacity>(
        find.descendant(of: find.byKey(Key('app-nav-label-motion-${destination.name}')), matching: find.byType(AnimatedOpacity)),
      )
      .opacity;
}
...
double _iconScale(WidgetTester tester, AppNavigationDestination destination) {
  return tester
      .widget<AnimatedScale>(
        find.descendant(of: find.byKey(Key('app-nav-icon-motion-${destination.name}')), matching: find.byType(AnimatedScale)),
      )
      .scale;
}
