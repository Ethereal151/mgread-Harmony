...
核心本地阅读业务已接入 OHOS 沙箱和 HAP 内置 SQLite，并已在 API 26 x86_64 模拟器完成安装、冷启动、首次启动集成测试和空书架加载回归；缓存、进度及完整业务真机回归仍需继续。音频/视频已完成原生 bridge 的代码和 HAP 编译接入，但真实媒体、后台会话和首帧仍待设备证据；在线数据源等待 Node 24.16.0 OHOS arm64 PoC。扫码已在 arm64 真机拉起 HMS ScanKit 并验证取消生命周期，有效二维码业务路由仍待验证。
