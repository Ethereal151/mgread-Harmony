...
这是 MgRead 的 OHOS 功能基线矩阵。标记为“最低交付”的能力已通过代码测试或 API 26 x64 模拟器回归；完整 Runtime 和原生媒体能力仍单独标注限制。
...
| 音频播放 | Flutter UI 和现有平台实现 | `packages/mg_read_audio_player/` | OHOS 入口安全降级，未接入原生播放 | 5 |
| 视频播放 | Flutter UI 和现有平台实现 | `packages/mg_read_video_player/` | OHOS 入口安全降级，未接入原生播放 | 5 |
| 扫码 | Android、Darwin、Web 实现 | `packages/mobile_scanner/` | OHOS 页面明确提示并不启动相机 | 5 |
| 局域网同步 | 已存在 | `lib/features/lan_sync/`、现有同步测试 | 未验证 | 2/3 |
| 文件选择、分享、链接、亮度、常亮 | 依赖三方 Flutter plugins | `lib/platform/`、根 `pubspec.yaml` | 能力层已接入；OHOS 缺失实现统一降级，亮度不展示假能力 | 3 |

## 阶段 0 结论

核心本地阅读业务已接入 OHOS 沙箱和 HAP 内置 SQLite，并已在 API 26 x86_64 模拟器完成安装、冷启动、首次启动集成测试和空书架加载回归；缓存、进度及 arm64 真机仍需后续真实设备回归。在线数据源等待 Node 24.16.0 OHOS arm64 PoC；音视频和扫码当前采用安全降级。
