...
| 在线数据源 / Node Runtime | Android/Windows 宿主和资源声明 | `packages/mgread_plugin_runtime/`、`integration_test/ohos_runtime_smoke_test.dart`、`integration_test/ohos_runtime_lifecycle_test.dart` | Node 24.16.0 arm64 真机 ping、原生 restart 后 Facade 恢复和生命周期诊断通过；ArkWeb smoke 与 5 个本地验收 fixture 全链路通过；至少 5 个真实来源和 Cookie/Profile 隔离仍待验收 | 1 |
