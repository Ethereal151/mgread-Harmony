...
以下门禁仍需保留为未完成：播放器代理边界、锁屏/蓝牙/焦点与中断恢复、文件导入/导出/反馈的真机 UI 闭环；有效二维码载荷路由、真实来源→书架→Reader 迁移、文件分享分发、五个真实来源直连链路、HLS 样本和完整的相关跨设备/HAP 传输矩阵已通过，OHOS↔OHOS 与不上架的 HAP 市场跳转已按用户要求跳过。
